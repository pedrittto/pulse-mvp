...
// V2 legacy tests - these should only test L/M/H categories
describe('Impact V2 Legacy Scoring', () => {
  // Set environment to ensure V2 mode
  const originalImpactMode = process.env.IMPACT_MODE;
  
  beforeAll(() => {
    // Ensure we're testing V2 mode (default when IMPACT_MODE is not 'v3')
  ...
  describe('threshold boundaries', () => {
    ...
    test('V2 should not produce Critical category', () => {
      const score = scoreNews({
      ...
      // V2 only supports L/M/H, never Critical
      expect(['L', 'M', 'H']).toContain(score.impact);
      expect(score.impact).not.toBe('C');
      expect(score.impact_score).toBeGreaterThanOrEqual(70);
    });
  });
